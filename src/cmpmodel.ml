...
type d_dataModule = d_dataType list
...
type d_contractModule = d_interface list
...
type d_componentModule = d_component list
...
type d_deploymentModule = d_component_ref list * d_connection list
...
type d_model = {
  data : d_dataModule;
  contracts : d_contractModule;
  components :  d_componentModule;
  deployment : d_deploymentModule;
} [@@deriving sexp]
